...
const browserEventListeners = new Map();
export function emitBrowserEvent(event, payload) {
...
export async function invoke(cmd, args) {
    ...
    switch (cmd) {
        ...
        case 'save_prefs': {
            try {
                localStorage.setItem('ollalink_user_prefs', JSON.stringify(args?.prefs));
            }
            catch (e) {
        ...
        case 'list_audio_devices': {
            ...
            if (!inputs.length)
                inputs = ['Default Microphone (Browser)'];
            if (!outputs.length)
                outputs = ['Default Speaker (Browser)'];
            return {
                inputs,
                outputs,
                    ...
                    ws.onmessage = (e) => {
                        if (typeof e.data === 'string') {
                            ...
                            if (lastPacketTime > 0) {
                                ...
                                if (packetArrivalIntervals.length > 8)
                                    packetArrivalIntervals.shift();
                            }
                            lastPacketTime = now;
        ...
        case 'set_captions': {
            if (browserWs && browserWs.readyState === WebSocket.OPEN) {
                browserWs.send(JSON.stringify({ type: 'captions-toggle', on: args?.on }));
            }
            return undefined;
        }
        ...
        case 'change_languages': {
            if (browserWs && browserWs.readyState === WebSocket.OPEN) {
                browserWs.send(JSON.stringify({
                    type: 'lang.change',
                    sourceLang: args?.sourceLang || args?.source_lang,
                    targetLang: args?.targetLang || args?.target_lang,
                }));
            }
            return null;
        }
